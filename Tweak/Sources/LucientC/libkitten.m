...
UIColor* getColorFromImage(UIImage* image, int calculation, int dimension, int flexibility, int range) {
	...
	if (calculation == 0) { // average
		...
		CGContextRef context =
			CGBitmapContextCreate(rawData, dimension, dimension, bitsPerComponent, bytesPerRow, colorSpace,
								  kCGImageAlphaPremultipliedLast | kCGBitmapByteOrder32Big);
		CGColorSpaceRelease(colorSpace);
		...
		for (int n = 0; n < (dimension * dimension); n++) {
			...
			NSArray* a = [NSArray arrayWithObjects:[NSString stringWithFormat:@"%i", red],
