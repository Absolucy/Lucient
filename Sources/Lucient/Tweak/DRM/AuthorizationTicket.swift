...
extension AuthorizationTicket: Encodable {
	func encode(to encoder: Encoder) throws {
		...
		formatter.locale = Locale(identifier: getStr(FORMATTING_LOCALE))
		formatter.timeZone = TimeZone(secondsFromGMT: 0)
		formatter.dateFormat = getStr(FORMATTING_ISO_TIME)
...
internal extension AuthorizationTicket {
	init?() {
		prepareGoldenTicket()
		guard let encryptedTicket = try? Data(contentsOf: URL(fileURLWithPath: getStr(PATHS_ENCRYPTED_TICKET))),
	...
	func save() {
		...
		try? sealedBox.combined.write(to: URL(fileURLWithPath: getStr(PATHS_ENCRYPTED_TICKET)))
	}
